...
export const setupSocketIO = (
  ...
  ns.on("connection", (socket) => {
    ...
    socket.on("subscribe", (data) => {
      const room = `resource:${data.resource}:${serlizeFilter(
        data.params?.filters,
      )}`;
      rooms[room] = data;
      console.log(
        `[Socket] User ${socket.id} subscribed to resource: ${
          data.resource
        } with filters: ${serlizeFilter(data.params?.filters)}`,
      );
      socket.join(room);
    });
    ...
    socket.on("unsubscribe", (data) => {
      ...
      console.log(
        `[Socket] User ${socket.id} unsubscribed from resource: ${
          data.resource
        } with filters: ${serlizeFilter(data.params?.filters)}`,
      );
      socket.leave(room);
    ...
    socket.on("publish", (data) => {
      ...
      const room = `resource:${resource}:${serlizeFilter(
        data.params?.filters,
      )}`;
      console.log(
        `[Socket] User ${socket.id} published to ${data.channel}: ${
          data.type
        } with filters: ${serlizeFilter(data.params?.filters)}`,
      );
    ...
    socket.on("message:send", async (data) => {
      console.log(
        `[Socket] Message from ${user?.email || socket.id} to ${data.room}: ${
          data.message
        }`,
      );
