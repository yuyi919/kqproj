...
export function NightActionPanel({
  ...
  // 获取选中卡牌的完整信息
  const selectedCard = selectedCardRef ? getCardDefinition(selectedCardRef) : null;

  const handleUseCard = () => {
    if (selectedCardId) {
      // 结界不需要目标
      const targetId = selectedCard?.type === 'barrier' ? '' : (selectedTargetId || '');
                  ...
                  >
                    <Space orientation="vertical" size={0} align="start">
                      <Space>
                        ...
                        {cardRef.type === 'kill' && (
                          <Badge count={killMagicAvailable} color={token.colorError} />
                        )}
                      </Space>
