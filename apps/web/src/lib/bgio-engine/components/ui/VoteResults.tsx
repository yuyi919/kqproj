 ...
 * 投票结果展示组件
 */
export function VoteResults({ players, voteCounts }: VoteResultsProps): React.ReactElement {
          ...
          return (
            <List.Item>
              <Space style={{ width: '100%', justifyContent: 'space-between' }}>
                <Text>{player ? `${player.seatNumber}号玩家` : '未知玩家'}</Text>
                <Tag color={count === sortedResults[0][1] ? 'success' : 'default'}>
                  {count} 票
                  {count === sortedResults[0][1] && ' 👑'}
                </Tag>
              </Space>
            </List.Item>
          );
