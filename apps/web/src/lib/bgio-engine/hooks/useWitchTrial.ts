...
import type { BGGameState, Card, CardRef, PrivatePlayerInfo, CardType } from '../types';
...
export function useWitchTrial(props: BoardProps<BGGameState>): UseWitchTrialReturn {
  ...
  // 工具函数
  const getCardByType = useCallback((type: CardType): CardRef | undefined => {
    return mySecrets?.hand.find(c => c.type === type);
  }, [mySecrets]);
  
  const hasCardType = useCallback((type: CardType): boolean => {
    return mySecrets?.hand.some(c => c.type === type) || false;
  }, [mySecrets]);
  
  const canUseCard = useCallback((card: Card): boolean => {
    // 持有魔女杀手时只能使用魔女杀手
