...
import { WitchTrialGame, WitchTrialBoard } from '@/lib/bgio-engine';
...
import { useWitchTrial } from '@/lib/bgio-engine';
...
function CustomBoard(props) {
  ...
  return (
    <div>
      <header>
        <h1>第 {game.round} 天 - {game.phase}</h1>
...
interface BGGameState {
  ...
  players: Record<string, BGPlayerState>;  // 玩家状态
  secrets: Record<string, PlayerSecrets>;  // 秘密信息（只有对应玩家可见）
  deck: Card[];           // 牌堆
  discardPile: Card[];    // 弃牌堆
  currentActions: Record<string, PlayerAction>;  // 当前行动
  currentVotes: Vote[];   // 当前投票
  deathLog: DeathRecord[];  // 死亡记录
  currentImprisonedId: string | null;  // 被监禁玩家
  attackQuota: {
...
│   ├── NightActionPanel.tsx  # 夜间行动
│   └── PhaseDisplay.tsx  # 阶段显示
├── hooks/                # React Hooks
│   └── useWitchTrial.ts  # 游戏状态 Hook
├── example.tsx           # 使用示例
└── README.md             # 本文档
```
