...
export function runCompleteGameExample(): void {
  ...
  if (result) {
    ...
    result.deathLog.forEach((death) => {
      ...
      console.log(
        `      第${death.round}天: ${death.playerId} - ${causeMap[death.cause]}`,
      );
    });
  }
...
export function demonstrateInfoHiding(): void {
  ...
  for (const [id, player] of gmState.players) {
    console.log(`\n玩家 ${id}:`);
    console.log(
      `  状态: ${player.status},  魔女化: ${player.isWitch},  手牌数: ${player.hand.length},  结界: ${player.hasBarrier},  死因: ${player.deathCause || "N/A"}`,
    );
  }
