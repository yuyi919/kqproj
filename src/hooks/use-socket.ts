...
export const useSocket = () => {
  ...
  const [lastMessage, setLastMessage] = useState<MessageReceivedPayload | null>(null)
  const [logs, setLogs] = useState<{ type: string; payload: any; timestamp: number }[]>([])

  const addLog = useCallback((type: string, payload: any) => {
    setLogs((prev) => [{ type, payload, timestamp: Date.now() }, ...prev].slice(0, 50))
  }, [])
    ...
    fetch("/api/socket", { cache: "no-store" }).finally(() => {
      ...
      socket.on("auth:status", (status: AuthStatusPayload) => {
        if (status.authenticated && status.user) {
          setUser(status.user)
          addLog("auth:status", { authenticated: true, userEmail: status.user.email })
        } else {
          setUser(null)
