...
export function handleSetupPhase(state: GameState): PhaseResult {
  ...
  for (const player of state.players.values()) {
    if (player.witchKillerHolder) {
      ...
      events.push(
        createEvent(GameEventType.WITCH_TRANSFORM, {
          playerId: player.id,
...
export function handleMorningPhase(state: GameState): PhaseResult {
  ...
  // 检查上一轮的死亡情况
  const recentDeaths = state.deathLog.filter(
    (d) => d.round === state.round - 1,
  );

  for (const death of recentDeaths) {
    events.push(
      createEvent(GameEventType.PLAYER_DIE, {
...
export function endVotingPhase(state: GameState): PhaseResult {
  ...
  events.push(
    createEvent(GameEventType.VOTE_RESULT, {
      round: state.round,
  ...
  if (voteResult.imprisonedId) {
    console.log(
      `[投票结果] ${voteResult.imprisonedId} 被监禁，本回合无法使用手牌`,
    );
...
export function handleResolutionPhase(state: GameState): PhaseResult {
  ...
  const imprisonedId =
    lastVoteResult?.round === state.round ? lastVoteResult.imprisonedId : null;
  ...
  for (const player of wreckedPlayers) {
    events.push(
      createEvent(GameEventType.WRECK_TRANSFORM, {
        playerId: player.id,
        round: state.round,
      }),
    );
  ...
  switch (cardType) {
    ...
    case CardType.KILL: {
      ...
      if (alreadyKilled.has(targetId) || !isAlive(target)) {
        // 攻击落空
        events.push(
          createEvent(GameEventType.CARD_USED, {
            actorId,
            cardType,
            targetId,
            result: "missed",
      ...
      // 击杀成功
      const cause = isWitchKiller
        ? DeathCause.WITCH_KILLER
        : DeathCause.KILL_MAGIC;
