...
export const CARD_DEFINITIONS: Record<CardType, Omit<Card, "id">> = {
  [CardType.WITCH_KILLER]: {
    ...
    name: "魔女杀手",
    description:
      "对目标发动攻击（优先度最高），持有者魔女化。如果不使用，杀人魔法有3个名额；如果使用，杀人魔法只有2个名额。",
    consumable: false,
    priority: 100, // 最高优先级
  },
  [CardType.BARRIER]: {
  ...
  [CardType.KILL]: {
    ...
    name: "杀人魔法",
    description:
      "对目标发动攻击（优先度低于魔女杀手），成功击杀后魔女化。当晚最多3人可使用（魔女杀手使用时只有2个名额）。",
    consumable: true,
    priority: 80,
  },
  [CardType.DETECT]: {
    ...
    name: "探知魔法",
    description:
      "探知目标手牌总数并随机获悉其中一张名称。以目标使用手牌前的状态为准。",
    consumable: true,
    priority: 90, // 先于攻击结算
  },
  [CardType.CHECK]: {
...
export function validateCardUsage(
  ...
  if (
    !isNight &&
    (card.type === CardType.KILL || card.type === CardType.BARRIER)
  ) {
    throw new GameError(GameErrorCode.INVALID_PHASE, "该卡牌只能在夜间使用");
  }
}
...
export function removeCardFromHand(hand: Card[], cardId: string): Card {
  ...
  if (index === -1) {
    throw new GameError(GameErrorCode.CARD_NOT_FOUND, "手牌中找不到该卡牌");
  }
