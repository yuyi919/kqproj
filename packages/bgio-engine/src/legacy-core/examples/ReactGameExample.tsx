    ...
    <div className="player-list">
      ...
      {Object.entries(players).map(([id, player]) => {
        ...
        return (
          <div
            key={id}
            className={`player-item ${isSelf ? "self" : ""} ${
              isDead ? "dead" : ""
            } ${isSelected ? "selected" : ""}`}
    ...
    <div className="night-phase">
      ...
      {selectedCard && selectedTarget ? (
        <div className="action-confirm">
          <p>
            使用 {getCardName(player.hand, selectedCard)} 攻击 {selectedTarget}?
          </p>
      ...
      {player.hasBarrier && (
        <div className="barrier-status">🛡️ 你当前有结界保护</div>
      )}
    </div>
  );
}
...
function MorningPanel({
  ...
  const lastNightDeaths =
    deaths?.filter((d) => d.round === currentRound - 1) || [];
          ...
          <ul>
            {lastNightDeaths.map((death) => (
              <li key={death.playerId}>
                {death.playerId} 死亡
                <span className="death-cause-unknown">
                  （死因未知，需使用检定魔法查验）
                </span>
              </li>
            ))}
...
function HandPanel({
  ...
  return (
    <div className="hand-panel">
      <h3>🎴 我的手牌</h3>
      {isWitch && <div className="witch-badge">🧙 魔女化</div>}
      {hasBarrier && <div className="barrier-badge">🛡️ 结界保护中</div>}
      ...
      {!canAct && <div className="cannot-act">当前阶段无法使用手牌</div>}
    </div>
  );
}
