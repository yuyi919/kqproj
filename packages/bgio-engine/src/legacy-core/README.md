...
function GameRoom({ roomId, playerId }: { roomId: string; playerId: string }) {
  ...
  return (
    <div>
      <h1>
        第 {game.currentRound} 天 - {game.currentPhase}
      </h1>
      ...
      {game.currentPhase === "voting" && <VotingPhase onVote={game.vote} />}
    </div>
  );
}
```
...
- **魔女杀手不发动攻击**：当晚最多 3 个杀人魔法攻击名额
- **魔女杀手发动攻击**：当晚最多 2 个杀人魔法攻击名额

### 魔女化与残骸化

- **魔女化**：持有【魔女杀手】或使用【杀人魔法】成功击杀后获得
- **残骸化**：魔女化后连续 2 夜未击杀则残骸化死亡
- **魔女杀手转移**：
  - 残骸化死亡：随机分配给存活玩家
  - 被杀人魔法击杀：转移给击杀者
