...
- Structured error handling (Either, Option types) - **for complex patterns, use fp:effect-ts skill**
- Dependency injection patterns - **for full service architecture, use fp:effect-ts skill**
- Resource management
...
4. **Select appropriate tools** - es-toolkit for collections, effect for types/errors, fp:effect-ts skill for complex Effect-TS architecture
5. **Refactor incrementally** - Small steps, verify at each stage
6. **Verify correctness** - Run tests, check edge cases

### Using fp:effect-ts Skill

For Effect-TS-specific refactoring tasks, delegate to the `fp:effect-ts` skill:

**Route to fp:effect-ts when:**
...
// 2. Route to fp:effect-ts for service design
